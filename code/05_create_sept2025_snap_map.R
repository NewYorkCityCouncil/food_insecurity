...
borough = st_read("https://data.cityofnewyork.us/resource/gthc-hcne.geojson") %>%
...
pal = councildown::colorBin(palette = "cool",
                            bins = 4,
                            domain = d,
...
community_districts = community_districts %>%
  mutate(label = paste0("<strong>Community District #</strong>: ", BoroCD, "<br>",
                        ...
                        "<strong>Number of SNAP recipients (Sept 2024):</strong> ", 
                            format(bc_snap_recipients.x, big.mark = ","), "<br>",
                        "<strong>% of Population receiving SNAP (Sept 2025):</strong> ", round(perc_snap_cur*100, 0), "%<br>",
                        "<strong>% point change in recipients (since Sept 2023):</strong> ", 
...
map = leaflet() %>% 
  ...
  addLegend_decreasing(position = "topleft", pal = pal2, 
                     ...
                     title = paste0("% point change in SNAP recipients <br>", 
                                    "from Sept 2023 to Sept 2025"), 
                     labFormat = labelFormat(suffix = "%"),
                     opacity = 1, decreasing = T, 
                     group = "% point change in SNAP recipients") %>%
  addLayersControl(
    baseGroups = c("% SNAP Sept 2025", "% point change in SNAP recipients"),
...
map = leaflet() %>% 
  addPolygons(data = community_districts, weight = 0, color = ~pal2(perc_point_change_snap), 
              fillOpacity = 1, smoothFactor = 0, popup = ~label, 
              group = "% point change in SNAP recipients since Sept 2019") %>%
  addCouncilStyle(add_dists = T, 
                  highlight_dists = c(11:18, 20, 24, 27:28, 37, 42, 44, 45, 47, 48, 50)) %>%
  addLegend_decreasing(position = "topleft", pal = pal2, 
                       values = community_districts$perc_point_change_snap,
                       title = paste0("% point change in SNAP recipients <br>", 
                                      "from Sept 2019 to Sept 2024"), 
                       labFormat = labelFormat(suffix = "%"),
                       opacity = 1, decreasing = T, 
                       group = "% point change in SNAP recipients")
...
community_districts %>%
  ...
  rename(`Community District` = BoroCD, 
         `2020 Population` = pop, 
         `Sept 2025 SNAP recipients` = bc_snap_recipients.x, 
