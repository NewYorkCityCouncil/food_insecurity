...
# prep for plotting ------------------------------------------------------------
d = c(min(nta_2020$food_insecurity_2024, na.rm=T), 
      max(nta_2020$food_insecurity_2024, na.rm=T))

quantile(nta_2020$food_insecurity_2024, c(0, .2, .4, .6, .8, 1), na.rm=T)*100
pal = colorBin(
...
nta_2020 = nta_2020 %>%
  mutate(label = paste0("<strong>NTA</strong>: ", nta2020, "<br>",
                        ...
                        "<strong>% food insecure in 2024:</strong> ", round(food_insecurity_2024*100, 1), "%<br>", 
                        "<strong>% point change in food insecurity 2023-2024</strong>: ", 
...
map = leaflet() %>% 
  ...
  addPolygons(data = nta_2020, weight = 0, color = ~pal(food_insecurity_2024), 
              fillOpacity = 1, smoothFactor = 0, popup = ~label, 
              group = "% food insecure 2024") %>% 
  addLegend_decreasing(position = "topleft", pal = pal, 
                       values = nta_2020$food_insecurity_2024,
                       ...
                       group = "% food insecure 2024") %>%
  addLayersControl(
    baseGroups = c("% food insecure 2024", "% point change in food insecurity since 2023"),
