...
community_districts = community_districts %>%
  mutate(label = paste0("<strong>Community District #</strong>: ", BoroCD, "<br>",
                        ...
                        "<strong>% of Population receiving SNAP (Sept 2024):</strong> ", round(perc_snap_cur*100, 0), "%<br>",
                        "<strong>% point change in recipients (since Sept 2019):</strong> ", 
...
map = leaflet() %>% 
  ...
  addLegend_decreasing(position = "topleft", pal = pal2, 
                     ...
                     group = "% point change in SNAP recipients") %>%
  addLayersControl(
    baseGroups = c("% SNAP Sept 2024", "% point change in SNAP recipients"),
