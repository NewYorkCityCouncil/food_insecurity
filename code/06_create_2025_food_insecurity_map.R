...
# https://public.tableau.com/app/profile/claire.reynolds6296/viz/SupplyGap2024/SupplyGap2024
################################################################################
...
# create palette for food insecurity
d = c(min(nta_2020$food_insecurity_2025, na.rm=T), 
      max(nta_2020$food_insecurity_2025, na.rm=T))
quantile(nta_2020$food_insecurity_2025, c(0, .2, .4, .6, .8, 1), na.rm=T)*100
pal = colorBin(palette = pal_nycc("warm"),
               bins = c(.02, .10, .15, .2, .36),
               domain = d,
               na.color = "grey")

# create palette for % pt change
d = c(min(nta_2020$perc_point_change_food_insecurity, na.rm=T), 
      max(nta_2020$perc_point_change_food_insecurity, na.rm=T))
quantile(nta_2020$perc_point_change_food_insecurity, c(0, .2, .4, .6, .8, 1), na.rm=T)*100
pal2 = colorBin(palette = rev(pal_nycc("diverging")),
                bins = c(-9, -5, -3, -1, 1, 3, 5, 9),
                domain = d*100,
                na.color = "grey")
...
nta_2020 = nta_2020 %>%
  mutate(label = paste0("<strong>NTA</strong>: ", nta2020, "<br>",
                        ...
                        "<strong>% food insecure in 2025:</strong> ", round(food_insecurity_2025*100, 1), "%<br>", 
                        "<strong>% point change in food insecurity 2023-2025</strong>: ", 
...
map = leaflet() %>% 
  ...
  addPolygons(data = nta_2020, weight = 0, color = ~pal2(perc_point_change_food_insecurity*100), 
              ...
              group = "% point change in food insecurity since 2023") %>% 
  ...
  addPolygons(data = nta_2020, weight = 0, color = ~pal(food_insecurity_2025), 
              fillOpacity = 1, smoothFactor = 0, popup = ~label, 
              group = "% food insecure 2025") %>% 
  addLegend_decreasing(position = "topleft", pal = pal, 
                       values = nta_2020$food_insecurity_2025,
                       ...
                       group = "% food insecure 2025") %>%
  addLayersControl(
    baseGroups = c("% food insecure 2025", 
                   "% point change in food insecurity since 2023"),
